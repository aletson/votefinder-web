...
def game(request, slug):
    ...
    if game.is_user_mod(request.user) and (
                game.last_vc_post == None or datetime.now() - game.last_vc_post >= timedelta(minutes=60) or (
        game.deadline and game.deadline - datetime.now() <= timedelta(minutes=60))):
        post_vc_button = True
    else:
...
def votecount(request, gameid):
    ...
    if game.is_user_mod(request.user) and (game.last_vc_post == None or datetime.now() - game.last_vc_post >= timedelta(
            minutes=60) or game.deadline - datetime.now() <= timedelta(minutes=60)):
        post_vc_button = True
    else:
        post_vc_button = False
...
def post_vc(request, gameid):
    ...
    if game.last_vc_post != None and datetime.now() - game.last_vc_post < timedelta(minutes=60) or game.deadline - datetime.now() > timedelta(minutes=60):
