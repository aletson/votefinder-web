	...
	def SearchLineForVotes(self, post, line):
		...
		while match:
			...
			try:
				...
				if game.ecco_mode == False or playersLastVote == None or (theLastGameDay == None and (playersLastVote == None or playersLastVote.unvote == True or v.unvote == True)) or (playersLastVote.unvote == False and playersLastVote.post > theLastGameDay.startPost) or (playersLastVote.unvote == True) or (v.unvote == True):
					v.save()
