...
class VotecountFormatter:    
    ...
    def go(self, show_comment = True):
        ...
        gameday = self.game.days.select_related(depth=1).all().order_by('-dayNumber')[:1][0]
        ...
        for item in self.results:
            ...
            if len(item['votes']) > 0:
                ...
                for v in item['votes']:
                    ...
                    if v['unvote']:
                        if detail_level == 3:
                            thisvote = ''.join([game_template.before_unvote, str(v['author']), game_template.after_unvote])
                    elif v['enabled']:
                        thisvote = ''.join([game_template.before_vote, str(v['author']), game_template.after_vote])
                    else:
                        if detail_level >= 2:
                            thisvote = ''.join([game_template.before_unvoted_vote, str(v['author']), game_template.after_unvoted_vote])
                    
                    if thisvote:
                        votelist.append(thisvote.replace("{{url}}", v['url']))
                            
                if len(votelist) > 0:
                    this_line = game_template.single_line.replace("{{target}}", str(item['target'])).replace("{{count}}", str(item['count'])).replace("{{votelist}}", ', '.join(votelist))
                    votecount_lines.append(this_line.replace("{{ticks}}", self.build_ticks(item['count'], self.tolynch)))

        self.not_voting_list = sorted(filter(lambda x: self.vc.currentVote[x] == None and x in living_players, self.vc.currentVote), key=lambda x: x.name.lower())
        temp_not_voting = game_template.single_line.replace("{{target}}", "Not Voting").replace("{{count}}", str(len(self.not_voting_list))).replace("{{ticks}}", self.build_ticks(0, self.tolynch))
        ...
        temp_overall = temp_overall.replace("{{notvoting}}", temp_not_voting.replace("{{votelist}}", ', '.join(map(lambda x: x.name, self.not_voting_list))))

        if len(comments) > 0:
                temp_overall += "\n \n" + "\n \n".join([c.comment for c in comments])
        
        self.bbcode_votecount = temp_overall.replace('{{deadline}}', str(deadline)).replace("{{timeuntildeadline}}", until_deadline).replace('{{day}}', str(gameday.dayNumber)).replace('{{tolynch}}', str(self.tolynch)).replace('{{alive}}', str(alive))
    ...
    def build_ticks(self, ticked, total):
        return ''.join(['[img]%s[/img]' % (self.empty_tick if i < (total - ticked) else self.tick) for i in range(0, total)])
